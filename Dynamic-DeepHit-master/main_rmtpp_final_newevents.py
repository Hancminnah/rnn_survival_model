...
import import_data_rmtpp as impt

from class_DeepLongitudinal_final_newevents import Model_Longitudinal_Attention
...
data_mode                   = comp_str + '_rmtpp_newevents_set' + str(set_str) 
seed                        = 1234
...
if fload == 0:
    (x_dim, x_dim_cont, x_dim_bin), (data, time, label), (mask1, mask2, mask3), (data_mi), target = impt.import_dataset('./data/mimic_'+comp_str+'_cvd.csv', norm_mode = 'standard')
    TUPLE3 = (x_dim, x_dim_cont, x_dim_bin, data, time, label, mask1, mask2, mask3, data_mi,target)
else:
    sfile = bz2.BZ2File('mimic_'+comp_str+'_setstr'+str(set_str)+'_cvdnewevents_final.pickle','rb')
...
with open('./data/mimic_'+comp_str+'_label_cvd.pickle','rb') as f:
...
if fload == 0:
    ...
    sfile = bz2.BZ2File('mimic_'+comp_str+'_setstr'+str(set_str)+'_cvdnewevents_final.pickle','wb')
...
for itr in range(iteration):
    ...
    #a, b, c, d, e, f, g, h = model.check(DATA, MASK, MISSING, PARAMETERS, keep_prob, lr_train,tgt_mb)
...
aa, rmse_overall = pred_next_eventtime(sess, model, te_data, te_data_mi, te_label, te_tgt, pred_time, new_parser, 100)
...
# repeated_data = pd.read_csv('/home/minmin/StayHome/Dynamic-DeepHit-master/data/test_day.csv')
# repeated_test_nric = repeated_data['id'].unique()
...
# aa, rmse_overall = pred_next_eventtime(sess, model, re_te_data, re_te_data_mi, re_te_label, re_te_tgt, pred_time, new_parser, 5)
